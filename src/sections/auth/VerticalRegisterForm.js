...
export default function VerticalRegisterForm() {
    ...
    const [identificationCard, setidentificationCard] = useState(null)
    const [businessChamber, setbusinessChamber] = useState(null)
    ...
    const RegisterSchema = Yup.object().shape({
        ...
        identificationCard: Yup.mixed().test('fileSize', 'El tamaño máximo de la imagen es de 60MB', (value) => {
            if (!value) return true
            return value.size <= 60 * 1024 * 1024;
        }),
        businessRegistration: Yup.mixed().test('fileSize', 'El tamaño máximo de la imagen es de 60MB', (value) => {
            if (!value) return true;
            return value.size <= 60 * 1024 * 1024;
        }),
        businessChamber: Yup.mixed().test('fileSize', 'El tamaño máximo de la imagen es de 60MB', (value) => {
            if (!value) return true;
            return value.size <= 60 * 1024 * 1024;
        }),
    });
                ...
                <Stack>
                    ...
                    <Card sx={{ maxWidth: "300px" }}>
                        ...
                        <CardContent sx={{ maxHeight: "400px" }}>
                            <Upload file={identificationCard} onDrop={indentificationCardRender} onDelete={() => setidentificationCard(null)} />
                        </CardContent>
                    </Card>
                </Stack>
            ),
            ...
            description: (
                <Stack>
                    <Card sx={{ maxWidth: "300px" }}>
                        ...
                        <CardContent sx={{ maxHeight: "400px" }}>
                            <Upload file={businessChamber} onDrop={businessChamberRender} onDelete={() => setbusinessChamber(null)} />
                        </CardContent>
                    </Card>
                </Stack>
            ),
            ...
            description: (
                <Stack>
                    <Card sx={{ maxWidth: "300px" }}>
                        ...
                        <CardContent sx={{ maxHeight: "400px" }}>
                            <Upload file={businessRegistration} onDrop={businessRegistrationRender} onDelete={() => setbusinessRegistration(null)} />
                        </CardContent>
                    </Card>
                </Stack>
            ),
            ...
            <Container maxWidth="sm">
                <Stepper activeStep={activeStep} orientation="vertical">
                    {steps.map((step, index) => (
                        <Step key={index}>
                            <StepLabel optional={index === 5 ? <Typography variant="caption">Último paso</Typography> : null}>
                                {step.label}
                            </StepLabel>
                            <StepContent>
                                {step.description}
                                <Box sx={{ mt: 3 }}>
                                    
                                    {
                                        index === steps.length - 1 ? (
                                            <Button variant="contained" onClick={handleSubmit(onSubmit)}>Finalizar</Button>
                                        ) : (
